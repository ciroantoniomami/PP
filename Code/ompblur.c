...
int main (int argc , char * argv[])
{
    ...
    new_M = (unsigned short int*)malloc(N[0] * N[1] * sizeof(unsigned short int)) ;  
    int nthreads ;
    ...
    //internal image
    #pragma omp for reduction(+:pixel) schedule(dynamic)  nowait
        for(int i = kdim ; i < N[1] - kdim ; i++){
          for(int j = kdim ; j < N[0] - kdim ; j++){
        new_M[i * N[0] + j] = 0;
        pixel=0;
        #pragma simd
            ...
            #pragma simd
              for(int v = 0 ; v < dim_kernel ; v ++){
        	      pixel += (M[(i + kdim - u) * N[0] + j + v - kdim] * K[(dim_kernel - 1 - u) * dim_kernel + v]);
               }
             }
             new_M[i * N[0] + j]=pixel;
          }
    ...
    #pragma omp for reduction(+:pixel) schedule(dynamic) nowait
        ...
        pixel=0;
        #pragma simd
             for(int u = kdim - i ; u < dim_kernel ; u ++){
            #pragma simd
              for(int v = 0 ; v < dim_kernel ; v ++){
        	        pixel += (M[(i + kdim - u) * N[0] + j + v - kdim] * K[(dim_kernel - 1 - u) * dim_kernel + v]);
               }
    ...
    //halo down
    #pragma omp for reduction(+:pixel) schedule(dynamic) nowait
        for(int i = N[1] - kdim ; i < N[1] ; i++){
          for(int j = kdim ; j < N[0] - kdim; j++){
        new_M[i * N[0] + j] = 0;
        pixel=0;
        #pragma simd
             for(int u = 0 ; u < kdim + N[1] - i ; u ++){
            #pragma simd
              for(int v = 0 ; v < dim_kernel ; v ++){
                pixel += (M[(i + kdim - u) * N[0] + j + v - kdim] * K[(dim_kernel - 1 - u) * dim_kernel + v]);
               }
    ...
    #pragma omp for reduction(+:pixel) schedule(dynamic)  nowait
        for(int i = kdim ; i < N[1] - kdim ; i++){
          for(int j = N[0] - kdim; j < N[0]; j++){
        new_M[i * N[0] + j] = 0;
        pixel=0;
        #pragma simd
             for(int u = 0 ; u < dim_kernel ; u ++){
            #pragma simd
              for(int v = 0 ; v < kdim + N[0] - j ; v ++){
                  pixel += (M[(i + kdim - u) * N[0] + j + v - kdim] * K[(dim_kernel - 1 - u) * dim_kernel + v]);
               }
             }
             new_M[i * N[0] + j]=pixel;
    ...
    #pragma omp for reduction(+:pixel) schedule(dynamic) nowait
        ...
        #pragma simd
             for(int u = 0 ; u < dim_kernel ; u ++){
            #pragma simd
              for(int v = kdim - j ; v < dim_kernel ; v ++){
                pixel += (M[(i + kdim - u) * N[0] + j + v - kdim] * K[(dim_kernel - 1 - u) * dim_kernel + v]);
               }
             }
             new_M[i * N[0] + j]=pixel;
    ...
    swap_image((void *)new_M , N[0] , N[1] , maxval);

    const char * new_image = argv[5] ;
    write_pgm_image((void*)new_M , maxval , N[0]  , N[1] , new_image) ;
...
//                    continue;
//                  else
//                   result_M[i * local_dim[0] + j] += (temp[ (i + (kdim - u)) * (local_dim[0]) + (j + (v - kdim))] *  K[(dim_kernel - 1 - u) * dim_kernel + v]) ;
//                 }
//               }
//            
//}