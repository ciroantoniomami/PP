...
int main (int argc , char * argv[])
{
    ...
    if (argc <=1){
        fprintf (stderr , " Required at least 4 parameters : "
         "type of the kernel , dimension of the kernel and weight (if needed) , input , output ") ;
    ...
    int dim_kernel ;
    int index = 3;
    type_kernel = atoi (argv[1]) ;
    dim_kernel = atoi (argv[2]) ;
    float weight = 1 ;
    if(type_kernel == 1){
    weight = atof (argv[3]) ;
    index = 4;
    }
    const char *image_name = argv[index] ;
...
//HALO DOWN-RIGHT-CORNER TO TOP-LEFT-CORNER
...
//HALO DOWN-LEFT-CORNER TO TOP-RIGHT-CORNER
 ...
 //HALO TOP-RIGHT-CORNER TO DOWN-LEFT-CORNER
...
//HALO TOP-LEFT-CORNER TO DOWN-RIGHT-CORNER
  ...
  #pragma omp parallel 
  {
   ...
   #pragma omp for schedule(static)
    ...
    if (rank == 0){
        ...
        const char * new_image = argv[index+1] ;
...
void kernel (double * K, int dim_kernel , int type_kernel , float weight)
{
    ...
    //Weighted Kernel = 1
    //Guassian Kernel = 2
    double f = (1 - weight)/(double)(dim_kernel*dim_kernel - 1) ;
    switch (type_kernel)
    {
    case 0 : 
        ...
        for(int i = 0 ; i < dim_kernel ; i++)
            for(int j = 0 ; j < dim_kernel ; j++)
                K[i*dim_kernel + j] = 1 / (double)(dim_kernel * dim_kernel);
    ...
    case 1 :
        ...
        for(int i = 0 ; i < dim_kernel ; i++)
            ...
            K[(dim_kernel/2)*dim_kernel + dim_kernel/2] = weight;
